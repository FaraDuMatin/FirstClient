...
// /practice — the live demo path: clock in -> briefing call -> scope ->
// upload logo -> revision call (client reacts to the real file) -> debrief.
// Papers, Please-INSPIRED look; all state changes go through the API routes.
...
const BUBBLE_BY_STATE: Record<Gig["state"], string> = {
  briefing:
...
export default function Practice() {
  ...
  const clockIn = () =>
    run(async () => setGig(await api<Gig>("/api/gigs", { method: "POST" })));
    ...
    >
      ...
      <section className="relative flex flex-1 items-center justify-center gap-8 overflow-hidden px-6 py-10">
        ...
        <div className="relative flex max-w-sm flex-col gap-4">
          ...
          {gig && state === "briefing" && (
            <CallPanel gigId={gig.id} stage="briefing" onCallEnded={() => void advance("scoping", { detail: "briefing call ended" })} />
          )}
      ...
      >
        ...
        <article className="w-80 -rotate-1 border-2 border-[#b3a988] bg-[#e8e0c9] p-5 text-[#3a3532] shadow-[10px_10px_0_0_rgba(20,17,17,0.55)]">
          ...
          <dl className="mt-3 space-y-1 text-xl leading-6">
            <div className="flex justify-between gap-4"><dt className="text-[#8a7f60]">CLIENT</dt><dd>Marcus Webb</dd></div>
            <div className="flex justify-between gap-4"><dt className="text-[#8a7f60]">BUSINESS</dt><dd>Beanhound Coffee</dd></div>
            <div className="flex justify-between gap-4"><dt className="text-[#8a7f60]">JOB</dt><dd>Logo design</dd></div>
            <div className="flex justify-between gap-4"><dt className="text-[#8a7f60]">BUDGET</dt><dd>Small</dd></div>
            <div className="flex justify-between gap-4"><dt className="text-[#8a7f60]">DEADLINE</dt><dd>Fast</dd></div>
          </dl>
          ...
          <p className="mt-3 text-right text-sm tracking-widest text-[#8a7f60]">{gig?.id ?? "8733H-P1410"}</p>
        </article>
        ...
        {scorecard && (
          <article className="w-96 -rotate-1 border-2 border-[#b3a988] bg-[#e8e0c9] p-5 text-[#3a3532] shadow-[10px_10px_0_0_rgba(20,17,17,0.55)]">
            ...
            {gig && (
              <Link
                href={`/gig/${gig.id}`}
                className={`${button} mt-4 inline-block bg-[#4a5a6e] text-center text-[#e8e0c9]`}
              >
                VIEW GIG TRANSCRIPT
              </Link>
            )}
      ...
      <footer className="flex items-center justify-between border-t-4 border-[#141111] bg-[#171414] px-4 py-3 font-[family-name:var(--font-pixel-heading)] text-[10px] tracking-widest text-[#a89f8a]">
        ...
        <span className="text-[#c9a84c]">DAY 1 · {(state ?? "OFF DUTY").toUpperCase()}</span>
